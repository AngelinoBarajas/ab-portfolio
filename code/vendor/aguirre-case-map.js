/* AB Portfolio vendor copy of the Daniel Aguirre Law case-results map (code backup/daniel-aguirre/case-results-map.js, 2026-09-25).
   Loaded lazily by ab-mission.js for the "Live map" monitor channel, with the inline sample cases.
   Portfolio patch, marked [AB]: window.__daMapNoLinks drops the popup's "Read full case" link (those pages live on the firm's site). */
/* ============================================================
  ...
  function init() {
    ...
    function renderSingleCase(c) {
      ...
      popupBody.innerHTML = back +
        ...
        (window.__daMapNoLinks ? '' : '<a class="popup-link" href="' + escHtml(c.slug) + '"><span>Read full case</span>' + SVG_ARROW + '</a>'); // [AB]
    }
