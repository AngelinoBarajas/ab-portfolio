...
3. Webflow MCP (`data_scripts_tool`): `register_hosted_script` with the **same display name** and the new version (`update_registered_script` 404s), then `add_site_script` / `add_page_script` with that version. CSS: `set_site_freeform_code` / page head `<link>` with the new URL + integrity. (Tried 2026-09-26: loading the bundles from freeform footer code with `defer` instead; Angelino chose to keep the registry.)
...
| GSAP 3.13 (+ ScrollTrigger, Draggable, InertiaPlugin, SplitText, ScrambleText, Flip), Lenis 1.3.26 | jsDelivr npm | Site scripts, footer, first | Site scripts, footer, in that order |
| `ab-core.prod.js` | `src/core/*.js` | Site scripts, footer, after Lenis |
| `ab-home.prod.js` | `src/home/*.js` | Home page footer script |
| `ab-work.prod.js` | `src/work/*.js` | Work page (Mission archive) footer script |
| `ab-mission` / `ab-services` / `ab-about` (+ `.css`) | `src/mission`, `src/services`, `src/about` | Mission template / Services template / About page footer script + page head `<link>` |
| `ab-404.prod.js` + `ab-404.prod.css` | `src/404/`, `src/ab-404.css` | 404 utility page footer script + page head `<link>` |
| `ab-process.prod.js` + `ab-process.prod.css` | `src/process/`, `src/ab-process.css` | Process page footer script + page head `<link>` |
| `ab-hub.prod.js` + `ab-hub.prod.css` | `src/hub/`, `src/ab-hub.css` | Services hub (`/services`) page footer script + page head `<link>` |
