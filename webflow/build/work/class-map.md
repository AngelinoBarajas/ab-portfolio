...
| `#arcGrid article.mcard` | Collection List **Missions** (sort asc) `#arcGrid.ab_arc_grid-wrap` › `.ab_arc_grid` (list) › `.ab_mission-card` (item) › `.ab_mission-card_inner` (cover, body, hidden color nodes) + `a.ab_mission-card_link[data-card-link][data-selectable][data-slug][data-name][aria-label][data-status][data-cover-kind]` | the item is the card (filter/Flip/tilt act on it). The link is an **empty overlay** (absolute, inset 0, z 5): Webflow won't put a Collection List (the nested Types tags) inside a Link block |
